...
#temp.seq <- seq(quantile(data$tmean, 0.01, na.rm = TRUE), quantile(data$tmean, 0.99, na.rm = TRUE), by = 0.1)
temp.seq <- seq(-5, 28, by=0.1)
...
f2uk <- ggplot(dfpred, aes(x, y, color=type)) + 
  ...
  scale_y_continuous(breaks = seq(1, 1.7, by = 0.1), labels = seq(1, 1.7, by = 0.1)) +
  ...
  coord_cartesian(xlim = c(-5, 30), ylim = c(0.98, 1.7))+
...
f2uk_p <-ggplot(dfpred, aes(x, y, color = type)) + 
  geom_hline(yintercept = 1, size = 0.25) +
  geom_vline(aes(xintercept = cen, color = type), linetype = "dashed", size = 0.25, show.legend = FALSE) +
  geom_line() +
  geom_ribbon(aes(ymin = lo, ymax = hi, fill = type), alpha = .2, size = 0.05, show.legend = FALSE) +
  ...
  scale_y_continuous(breaks = seq(1, 1.4, by = 0.1), labels = seq(1, 1.4, by = 0.1), limits=c(0.98, 1.4)) +
  scale_color_manual(values = colors2[c(3, 4)], labels = c("One-stage", "Two-stage")) +
  scale_fill_manual(values = colors2[c(3, 4)], labels = c("One-stage", "Two-stage")) +
  labs(x = "Percentile (corresponding temperature, °C)", y = "Relative Risk", color = NULL, fill = NULL) +
  theme_cc()
